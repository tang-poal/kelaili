...
(function (window, undefined) {
  ...
  jQuery.support = function () {
    ...
    jQuery(function () {
      ...
      if (typeof div.style.zoom !== "undefined") {
        ...
        support.shrinkWrapBlocks = div.offsetWidth !== 3; // Prevent IE 6 from affecting layout for positioned elements #11048
    ...
    setDocument = Sizzle.setDocument = function (node) {
      ...
      sortOrder = docElem.compareDocumentPosition ? function (a, b) {
        ...
        var cur,
            ...
            bup = b.parentNode,
            ap = [a],
            bp = [b]; // The nodes are identical, we can exit early

        if (a === b) {
          hasDuplicate = true;
          return 0; // Fallback to using sourceIndex (in IE) if it's available on both nodes
        } else if (a.sourceIndex && b.sourceIndex) {
          return (~b.sourceIndex || MAX_NEGATIVE) - (contains(preferredDoc, a) && ~a.sourceIndex || MAX_NEGATIVE); // Parentless nodes are either documents or disconnected
        } else if (!aup || !bup) {
  ...
  function fixCloneNodeIssues(src, dest) {
    var nodeName, data, e; // We do not need to do anything for non-Elements
    ...
    ajax: function (url, options) {
      ...
      function done(status, nativeStatusText, responses, headers) {
        ...
        if (status >= 200 && status < 300 || status === 304) {
          ...
          if (status === 304) {
            isSuccess = true;
            statusText = "notmodified"; // If we have data
          } else {
              ...
              try {
                ...
                if (callback && (isAbort || xhr.readyState === 4)) {
                  ...
                  if (isAbort) {
                    ...
                    status = xhr.status;
                    xml = xhr.responseXML;
                    responseHeaders = xhr.getAllResponseHeaders(); // Construct response list
  ...
  Tween.propHooks = {
    _default: {
      get: function (tween) {
        ...
        result = jQuery.css(tween.elem, tween.prop, "auto"); // Empty strings, null, undefined and "auto" are converted to 0.
